import { IQuestion, QuestionItemList } from '../dto';
import { IQuestionUsecase } from './question.usecase';
import { IQuestionCachingUsecase } from './questionCaching.usecase';

export class QuestionDummyUsecase implements IQuestionUsecase {
  constructor(private questionCaching: IQuestionCachingUsecase) {}
  ...
  getQuestionById = async (id: string) => {
    const question = this.questionCaching.getQuestion(id);
    if (question) {
      return question;
    }
    const x = this.dataDummy.find((el) => el.id === id);
    if (x === undefined) {
      throw new Error('Data Not Found');
    }
    this.questionCaching.setQuestion(x);
    return x;
  };

  getQuestionList = async () => {
    return this.dataDummy.map(
      (el) => new QuestionItemList({ id: el.id, status: 'unanswered' }),
    );
  };
}
